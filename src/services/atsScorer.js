...
function calculateKeywordMatch(resumeText, atsKeywords) {
  ...
  const score = (foundKeywords.length / atsKeywords.length) * 100;
...
function generateSuggestions(scores, resumeContent, jobAnalysis) {
  ...
  if (scores.keywordMatchScore < 70) {
    if (scores.missingKeywords.length > 0) {
      const topMissing = scores.missingKeywords.slice(0, 5);
      suggestions.push(`Add these important keywords to your resume: ${topMissing.join(', ')}`);
    }
    suggestions.push('Incorporate more ATS keywords naturally throughout your resume, especially in the summary and experience sections.');
  }

  // Skills coverage suggestions
  if (scores.skillsCoverageScore < 70) {
    if (scores.missingSkills.length > 0) {
      const topMissing = scores.missingSkills.slice(0, 5);
      suggestions.push(`Highlight these required skills: ${topMissing.join(', ')}. Add them to your skills section and demonstrate them in your experience bullets.`);
    }
    suggestions.push('Ensure all required skills from the job posting are mentioned in your resume.');
  }

  // Content quality suggestions
  if (scores.contentQualityScore < 70) {
    suggestions.push('Add more quantified achievements with specific metrics (numbers, percentages, dollar amounts) to your experience bullets.');
    suggestions.push('Start each experience bullet with a strong action verb (e.g., Developed, Implemented, Led, Optimized).');
    
    if (!resumeContent.summary || resumeContent.summary.length < 50) {
      suggestions.push('Expand your professional summary to 2-3 sentences that highlight your key qualifications.');
    }
  }
...
function scoreResume(resumeContent, jobAnalysis) {
  ...
  const overallScore = Math.round(
    (keywordMatch.score * 0.40) +
    (skillsCoverage.score * 0.35) +
